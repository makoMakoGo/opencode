...
**关键结论**: 93.2 分中 45 分来自三个满分维度（巨型文件、any 泛滥、v2 双写）。P0（v2 迁移）可降至 ~78，三个维度全部清零可降至 ~48。

### 补充指标（不计入评分）

- **循环依赖**: 20 模块 SCC（lsp↔project↔session 循环）→ 🔴
- **代码重复**: 543 块 / 1,181 实例（acp/agent.ts 最严重）→ 🟡
- **非空断言 (`!`)**: 173（集中在 CLI 层）→ 🟢
- **v0.x 不稳定依赖**: 19（bun-pty、ACP SDK）→ 🟡
- **补丁覆盖的包**: 4（solid-js 等）→ 🟢
...
**解读**: 45 分来自三个满分维度（巨型文件、any、双写）。P0（v2 迁移）后评分降至 **~78**（移除双写 15 分）。P0+P1 后降至 **~72**（any 从 717→444，省 ~1.7 分；deprecated 从 19→8，省 ~4.7 分）。需将三个满分维度全部清零（含拆分 16 个巨型文件）才能降至 ~48。
...
**推荐的投入顺序**：P0（v2 迁移）→ P1（console zen 类型化 + 废弃 API 清理）→ P2（测试回归修复 + 巨型文件拆分 + 循环依赖拆分）。P0 完成后评分降至 ~78（移除双写 15 分），P0+P1 降至 ~72（any 和 deprecated 进一步降低）。将三个满分维度全部清零（需同时拆分巨型文件）才能降至 ~48。
